...
test('Should order with 3 itens', () => {
  ...
  order.addItem(new Item(1, 'suplement', 'Omega 3', 70, dimensionDefault), 1)
  order.addItem(new Item(2, 'suplement', 'Spirulina', 90, dimensionDefault), 1)
  //then
...
test('Should calculate shipping based on item dimensions', () => {
  //given/when
  const order = new Order(cpf)
  order.addItem(new Item(1, 'instrument', 'Camera', 70, dimensionDefault), 1)
  order.addItem(new Item(2, 'suplement', 'Spirulina', 90, dimensionDefault), 1)
  //then
  expect(order.total).toEqual(160)
})
